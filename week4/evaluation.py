...
def voc_eval(predictions, annotations, ovthresh=0.5):
    ...
    fp = np.zeros(nd)
    for d in range(nd):
        frame_id = image_ids[d]
        if frame_id in list(class_recs.keys()):
            R = class_recs[frame_id]
...
def show_annotations_and_predictions(img, gt_rects, predictions):
    for gt_rect in gt_rects:
        img = cv2.rectangle(img, (int(gt_rect[0]), int(gt_rect[1])),  (int(gt_rect[2]), int(gt_rect[3])), GT_COLOR, 2)
    for prediction in predictions:
        img = cv2.rectangle(img, (int(prediction[0]), int(prediction[1])),  (int(prediction[2]), int(prediction[3])), PREDICTED_COLOR, 2)
    display_frame = cv2.resize(img, tuple(np.int0(0.5*np.array(img.shape[:2][::-1]))))
    cv2.imshow('Annotations / Predictions',display_frame)
    k = cv2.waitKey(1)
    if k == ord('q'):
        return

def eval_opticalflow(u, v, u_gt, v_gt, mask):
    # Remove pixels without ground truth
    u_gt, v_gt = u_gt[mask], v_gt[mask]
    u, v = u[mask], v[mask]

    # MSE for non-occluded
    err = (u - u_gt)**2 + (v - v_gt)**2

    err_image = np.zeros(mask.shape)
    err_image[mask] = err
    # plt.imshow(err_image, cmap='hot')
    mse = err.mean()
    pepn = np.sum(np.sqrt(err) > 3) / err.shape[0]
    return mse, pepn
